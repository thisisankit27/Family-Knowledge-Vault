...
| 4 — Family Memories (**17–20**) | Memories, Memory Photos, Voice Memories, Albums | 2h each | **Briefed in `docs/18-phase-4-brief.md`, 2026-08-17 — read it before PR-17.** ~~(16–20): Memories, Albums, Stories, Voice Memories, Memory Timeline~~ — **this row was wrong in three ways and all three are corrected there.** *The numbers:* PR-16 shipped as the landing page (see the Phase 3 row), so Phase 4 has four slots, not five. *The scope:* **Stories are a field, not a PR** — `docs/08` §4 already says *"Story is not an entity; it is a field"* — and **Memory Timeline moves to Phase 7**, where `src/navigation/domains.ts` already registers `timeline` as its own domain; Phase 4 ships the date a timeline needs, Phase 7 builds the surface once across every domain. *The package:* ~~Voice via Expo AV — natural fit.~~ **`expo-av` is unmaintained from SDK 54 and removed in SDK 55**, and this project pins `expo ~54.0.0`; nothing had validated that line against the pin. **Voice ships on `expo-audio`** — stable, included in Expo Go on 54, records `.m4a`/AAC. Video is deferred to Phase 12 with the 10MB file cap it depends on. "Reuses the upload pattern from Phase 3" survives intact and is the phase's main asset. |

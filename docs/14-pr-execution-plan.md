...
| 3 — Preserve What Matters (11–15) | Document Library, Categories, Viewer, Upload, **PR-15 vacated** | 2h, viewer maybe 2.5h | Upload via Expo ImagePicker/DocumentPicker + Supabase Storage. **All of §3.2's storage decisions and §6's four contradictions are settled — see `docs/16` §9 and `docs/17`.** PDF renders in a WebView; the demo stays on Expo Go and the dev build moves to Phase 10. PR-15 "Sharing" resolved to nothing Phase 3 should build; the slot is open, with export (`docs/17` §11) a candidate. |

...
- **Development runs against a local Supabase stack in Docker** (`npx supabase start`), not the hosted project — real Postgres, Auth, Storage and RLS, consuming no free-tier quota. `.env.local` selects it; renaming that file returns to hosted. Setup and the LAN-IP caveat are in the README, which also carries a status note: as of 2026-08-07 the stack is specified but has not been started once. **Storage is app-owned (Supabase Storage); bring-your-own-storage was reviewed and declined for Phase 3** — see `docs/17-storage-architecture-review.md`.
